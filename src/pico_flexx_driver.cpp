...
public:
  ...
  void callbackTopicStatus()
  {
    ...
    //Customization
    extraStatus = extraPublisher.getNumSubscribers() > 0;
...
private:
  ...
  void setTopics(const std::string &baseName, const int32_t queueSize)
  {
    ...
    //Customization
    extraPublisher = nh.advertise<sensor_msgs::PointCloud2>(baseName + PF_TOPIC_CLOUD + "_reduced", queueSize, cb, cb);
    //Customization
  ...
  void process()
  {
    ...
    while(running && ros::ok())
    {
      ...
      {
        extractData(*data, msgCameraInfo, msgCloud, msgCloudReduced, msgMono8, msgMono16, msgDepth, msgNoise, streamIndex);
        publish(msgCameraInfo, msgCloud, msgCloudReduced, msgMono8, msgMono16, msgDepth, msgNoise, streamIndex);
      }
      lockStatus.unlock();
  ...
  void extractData(const royale::DepthData &data, sensor_msgs::CameraInfoPtr &msgCameraInfo, sensor_msgs::PointCloud2Ptr &msgCloud, sensor_msgs::PointCloud2Ptr &msgCloudReduced,
                   ...
                   size_t streamIndex = 0) const
  {
    ...
    //msgCloudReduced->point_step = (uint32_t)(3 * sizeof(float));
    //msgCloudReduced->row_step = (uint32_t)(3 * sizeof(float) * data.width);
    //msgCloudReduced->fields.resize(3);
    ...
    //msgCloudReduced->data.resize(3 * sizeof(float) * data.points.size());
  ...
  void publish(sensor_msgs::CameraInfoPtr &msgCameraInfo, sensor_msgs::PointCloud2Ptr &msgCloud, sensor_msgs::PointCloud2Ptr &msgCloudReduced,
               ...
               size_t streamIndex = 0) const
  {
    ...
    //Customization
    if (extraStatus){
      extraPublisher.publish(msgCloudReduced);
      msgCloudReduced = sensor_msgs::PointCloud2Ptr(new sensor_msgs::PointCloud2);
    }
    //Customization
